...
import { 
  ...
  Activity, Tag, FileText, Info, Save, X
} from "lucide-react";
import { 
...
const CreateStatus = () => {
  ...
  // For fetching existing data in edit mode
  const { data: allStatuses } = useGetAllProjectStatusesQuery(undefined, { skip: !isEditMode });
  
  const [createStatus, { isLoading: isCreating }] = useCreateProjectStatusMutation();
  ...
  useEffect(() => {
    if (isEditMode && allStatuses?.data) {
      const status = allStatuses.data.find((s: any) => s.id === Number(id));
      if (status) {
        setForm({
          name: status.name || "",
          description: status.description || "",
          color_code: status.color_code || "#6366f1",
          is_active: status.is_active ?? true,
        });
      }
    }
  }, [isEditMode, allStatuses, id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name.trim()) {
      setStatusModal({ isOpen: true, type: "error", message: "Status name is required." });
      return;
    }

    try {
      if (isEditMode) {
        await updateStatus({ id: Number(id), data: form }).unwrap();
        setStatusModal({ isOpen: true, type: "success", message: "Workflow status updated successfully!" });
      } else {
        await createStatus(form).unwrap();
        setStatusModal({ isOpen: true, type: "success", message: "New workflow status defined!" });
      }
    } catch (err: any) {
      setStatusModal({ isOpen: true, type: "error", message: err?.data?.message || "Failed to save status." });
    }
  };
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", padding: "40px" }}>
      <div style={{ maxWidth: "800px", margin: "0 auto" }}>
        ...
        <button 
          ...
          style={{ display: "flex", alignItems: "center", gap: "8px", background: "none", border: "none", color: "#64748b", cursor: "pointer", fontWeight: 600, marginBottom: "24px", padding: "8px", borderRadius: "8px", transition: "background 0.2s" }}
          onMouseEnter={e => e.currentTarget.style.background = "#fff"}
          onMouseLeave={e => e.currentTarget.style.background = "none"}
        >
          <ArrowLeft size={18} /> Back to Workflow States
        </button>
        ...
        <div style={{ marginBottom: "32px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "8px" }}>
            ...
            <h1 style={{ fontSize: "1.5rem", fontWeight: 800, color: "#0f172a", margin: 0 }}>
              {isEditMode ? "Modify Status Definition" : "Define New Workflow State"}
            </h1>
          </div>
          <p style={{ color: "#64748b", margin: 0 }}>
            Workflow statuses control how projects and sites move through the system lifecycle.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: "grid", gap: "24px" }}>
          ...
          <div style={{ background: "#fff", borderRadius: "20px", border: "1px solid #e2e8f0", padding: "32px", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
            <div style={{ display: "grid", gap: "24px" }}>
              <InputField 
                ...
                placeholder="e.g. In Progress, Pending Review, Completed"
              />
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "16px", padding: "16px", background: "#f8fafc", borderRadius: "12px", border: "1px solid #f1f5f9" }}>
                <div style={{ display: "flex", gap: "12px", alignItems: "center", flex: 1 }}>
                  <div style={{ width: "40px", height: "40px", borderRadius: "10px", background: `${form.color_code}20`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <div style={{ width: "20px", height: "20px", borderRadius: "4px", background: form.color_code, border: "2px solid #fff", boxShadow: "0 0 0 1px #e2e8f0" }} />
                  </div>
                  <div>
                    <div style={{ fontSize: "0.9rem", fontWeight: 700, color: "#1e293b" }}>Status Color</div>
                    <div style={{ fontSize: "0.75rem", color: "#64748b" }}>Pick a color to represent this state in badges.</div>
                  </div>
                </div>
                <input 
                  type="color" 
                  ...
                  onChange={handleChange}
                  style={{ width: "40px", height: "40px", padding: 0, border: "none", background: "none", cursor: "pointer", borderRadius: "8px", overflow: "hidden" }}
                />
              </div>
              ...
              <div style={{ padding: "16px", background: "#f8fafc", borderRadius: "12px", border: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ display: "flex", gap: "12px" }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: "0.75rem", color: "#64748b" }}>Inactive statuses cannot be assigned to new records.</div>
                  </div>
                </div>
          ...
          <div style={{ background: "#eff6ff", borderRadius: "16px", padding: "20px", border: "1px solid #dbeafe", display: "flex", gap: "16px" }}>
            ...
            <div>
              <h4 style={{ margin: "0 0 4px 0", color: "#1e3a8a", fontSize: "0.9rem" }}>Naming Conventions</h4>
              <p style={{ margin: 0, color: "#3b82f6", fontSize: "0.8rem", lineHeight: 1.5 }}>
                Try to keep status names short and action-oriented. Common examples include "On Hold", "Maintenance", "Testing", or "Closed".
              </p>
            </div>
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: "12px", marginTop: "12px" }}>
            ...
            >
              {isLoading ? <><Loader2 size={18} style={{ animation: "spin 1s linear infinite" }} /> Saving...</> : <><Save size={18} /> {isEditMode ? "Update Definition" : "Define Status"}</>}
            </button>
          </div>
        </form>
      </div>
