...
import { 
  Plus, CheckCircle2, 
  AlertCircle, Files, FileType, Filter, RefreshCcw
} from "lucide-react";
import { 
  useGetAllDocumentTypesQuery, 
  useDeleteDocumentTypeMutation,
  useToggleDocumentTypeMutation
...
const DocumentTypeList = () => {
  ...
  const { data, isLoading, refetch } = useGetAllDocumentTypesQuery();
  const [deleteDocType, { isLoading: isDeleting }] = useDeleteDocumentTypeMutation();
  const [toggleDocType] = useToggleDocumentTypeMutation();
  ...
  const allDocTypes = data?.data || [];

  const filteredData = useMemo(() => {
    let result = allDocTypes;
    ...
    return result;
  }, [allDocTypes, statusFilter]);

  const statusLabel =
    statusFilter === "active" ? "Active Types"
  ...
  const handleToggle = async (item: any) => {
    try {
      await toggleDocType({ id: item.id, is_active: !item.is_active }).unwrap();
    } catch (err) {
      console.error("Failed to toggle document type:", err);
    }
  };
  ...
  const handleDelete = async () => {
    if (deleteModal.id) {
      try {
        await deleteDocType(deleteModal.id).unwrap();
  ...
  return (
    <div style={{ padding: "32px", maxWidth: "1200px", margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "32px" }}>
        <div>
          <h1 style={{ fontSize: "1.75rem", fontWeight: 800, color: "#0f172a", margin: 0 }}>Document Types</h1>
          <p style={{ color: "#64748b", marginTop: "4px", fontSize: "0.95rem" }}>
            Categorize system uploads like Citizenship, Contracts, and Site Photos.
          </p>
        </div>
        <button 
          onClick={() => navigate("/settings/document-types/create")}
          style={{ 
        ...
        >
          <Plus size={20} /> Create Doc Type
        </button>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "20px", marginBottom: "32px" }}>
        {[
          { label: "Defined Types", value: data?.data?.length || 0, icon: <Files size={20} />, color: "#6366f1" },
          { label: "Active Categories", value: data?.data?.filter((s: any) => s.is_active).length || 0, icon: <FileType size={20} />, color: "#10b981" },
        ].map((stat, idx) => (
      ...
      <DataTable
        ...
        columns={columns}
        isLoading={isLoading}
        onEdit={(item) => navigate(`/settings/document-types/edit/${item.id}`)}
      ...
      >
        <div style={{ textAlign: "center", padding: "20px" }}>
          ...
          <p style={{ color: "#64748b", fontSize: "0.95rem", marginBottom: "24px" }}>
            Are you sure you want to delete this document type? Records previously uploaded under this type may lose their categorization.
          </p>
