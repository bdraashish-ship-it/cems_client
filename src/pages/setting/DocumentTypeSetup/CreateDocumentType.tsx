...
const CreateDocumentType = () => {
  ...
  const { data: allItems } = useGetAllDocumentTypesQuery(undefined, { skip: !isEditMode });
  
  const [createItem, { isLoading: isCreating }] = useCreateDocumentTypeMutation();
  const [updateItem, { isLoading: isUpdating }] = useUpdateDocumentTypeMutation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name.trim()) {
      setStatusModal({ isOpen: true, type: "error", message: "Document type name is required." });
      return;
    }
    ...
    try {
      if (isEditMode) {
        ...
        setStatusModal({ isOpen: true, type: "success", message: "Document type updated successfully!" });
      } else {
        await createItem(form).unwrap();
        setStatusModal({ isOpen: true, type: "success", message: "New document type defined!" });
      }
    } catch (err: any) {
      setStatusModal({ isOpen: true, type: "error", message: err?.data?.message || "Failed to save document type." });
    }
  };

  const handleModalClose = () => {
    if (statusModal.type === "success") {
      navigate("/settings/document-types");
    }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8fafc", padding: "40px" }}>
      <div style={{ maxWidth: "800px", margin: "0 auto" }}>
        ...
        >
          <ArrowLeft size={18} /> Back to Document Types
        </button>
        ...
        <div style={{ marginBottom: "32px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "8px" }}>
            ...
            <h1 style={{ fontSize: "1.5rem", fontWeight: 800, color: "#0f172a", margin: 0 }}>
              {isEditMode ? "Modify Document Type" : "Define Document Type"}
            </h1>
          </div>
          <p style={{ color: "#64748b", margin: 0 }}>
            Categorize uploads to keep project documentation and employee records organized.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: "grid", gap: "24px" }}>
          <div style={{ background: "#fff", borderRadius: "20px", border: "1px solid #e2e8f0", padding: "32px", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
            <div style={{ display: "grid", gap: "24px" }}>
              <InputField 
                ...
                placeholder="e.g. Contract Agreement, Citizenship ID, Site Map"
              />
              ...
              <div style={{ padding: "16px", background: "#f8fafc", borderRadius: "12px", border: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ display: "flex", gap: "12px" }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: "0.75rem", color: "#64748b" }}>Only active types will be visible during file uploads.</div>
                  </div>
                </div>
          ...
          <div style={{ background: "#eff6ff", borderRadius: "16px", padding: "20px", border: "1px solid #dbeafe", display: "flex", gap: "16px" }}>
            ...
            <div>
              <h4 style={{ margin: "0 0 4px 0", color: "#1e3a8a", fontSize: "0.9rem" }}>Data Organization</h4>
              <p style={{ margin: 0, color: "#3b82f6", fontSize: "0.8rem", lineHeight: 1.5 }}>
                Proper document typing ensures that important files are easy to retrieve during audits or project reviews.
              </p>
            </div>
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: "12px", marginTop: "12px" }}>
            ...
            >
              {isLoading ? <><Loader2 size={18} style={{ animation: "spin 1s linear infinite" }} /> Saving...</> : <><Save size={18} /> {isEditMode ? "Update Type" : "Define Type"}</>}
            </button>
          </div>
        </form>
      </div>
